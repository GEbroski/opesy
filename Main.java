...
public class Main{
    ...
    public static Boolean canStillFormTeams(int totalCitizens, int superCitizenCount, int regularCitizenCount){
        if(totalCitizens < 4){ // If there are fewer than 4 citizens, teams cannot be formed
            return false;
        }
        if(superCitizenCount < 2 && regularCitizenCount < 2){ // If there are not enough Super and Regular Citizens to form teams, return false
            return false;
        }
        if(superCitizenCount == 0 || regularCitizenCount == 0){ // If there are no Super Citizens or no Regular Citizens, return false
            return false;
        }
        if(regularCitizenCount < 2){ // If there are fewer than 2 Regular Citizens, return false
            return false;
        }
        if(superCitizenCount < 2 && regularCitizenCount < 4){ // If there are fewer than 2 Super Citizens and fewer than 4 Regular Citizens, return true
            return true;
        }
